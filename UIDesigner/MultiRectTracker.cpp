...
CTrackerElement::CTrackerElement(CControlUI* pControl,int nType)
:m_pControl(pControl),m_nControlType(nType)
{
	m_pNotifier=NULL;
}
...
void CTrackerElement::AddNotifier(INotifyUI* pNotifier)
{
	ASSERT(m_pNotifier==NULL);
...
void CMultiControlTracker::Draw(CDC* pDC,LPSIZE pOffset/*=NULL*/)
{
	...
	for (int i=0;i<m_arrTracker.GetSize();i++)
	{
		...
		SetControlType(pArrTracker->m_nControlType);
...
BOOL CMultiControlTracker::MultiTrackHandle(CWnd* pWnd,CDC* pDCClipTo)
{
	...
	for (;;)
	{
		...
		case WM_MOUSEMOVE:
			...
			if(point.x<m_rcForm.left||point.x>m_rcForm.right||
				point.y<m_rcForm.top||point.y>m_rcForm.bottom)
			{
				bBeyond=TRUE;
...
CTrackerElement* CMultiControlTracker::FindTracker(CControlUI* pControl)
{
	for (int i=0;i<m_arrTracker.GetSize();i++)
	{
		CTrackerElement* pArrTracker = m_arrTracker.GetAt(i);

		if(pArrTracker->GetControl()==pControl)
			return pArrTracker;
	}
